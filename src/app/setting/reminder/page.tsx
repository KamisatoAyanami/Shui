...
export default function Home() {
  ...
  useEffect(() => {
    async function loadConfig() {
      ...
      let gold = val?.gold || goldList[0];

      try {
        const dbGoal = await invoke<number>("get_daily_goal");
        if (dbGoal && String(dbGoal) !== val?.gold) {
          gold = String(dbGoal);
        }
      } catch {}
  ...
  const saveConfig = async (
    ...
    if (filed === "gold") {
      try {
        await invoke("set_manual_goal", { goalMl: parseInt(value as string) });
      } catch {}
    }
